...
<c:if test="${pagedListHolder.pageCount > 1}">
	<div class="wi-pagination fox-pagination font-heading pagination-3">
		...
		<!-- KHONG PHAI TRANG DAU TIEN -->
		<c:if test="${!pagedListHolder.firstPage}">
			<a class="next page-numbers" href="<%=StringUtils.replace(pagedLink, "~", String.valueOf(pagedListHolder.getPage() - 1))%>"><span>Tiến</span></a>
		</c:if>
		
		<!-- TRANG DAU TIEN -->
		<c:if test="${pagedListHolder.firstLinkedPage > 0}">
			<a class="page-numbers" href="<%=StringUtils.replace(pagedLink, "~", "0")%>"><span>1</span></a></li>
		</c:if>
		...
		<c:if test="${!pagedListHolder.lastPage}">
			<li class="page-item"><a class="page-link"
				href="<%=StringUtils.replace(pagedLink, "~", String.valueOf(pagedListHolder.getPage() + 1))%>">Cuối</a></li>
		</c:if>
